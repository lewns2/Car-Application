...
def findfirst_thread():
    while 1:
        while FindFirstMode:
            findfirst.run()
        time.sleep(0.2)

def findsecond_thread():
    while 1:
        while FindSecondMode:
            findsecond.run()
        time.sleep(0.2)
...
def run():
    global direction_command, turn_command, pos_input, catch_input, cir_input, ultrasonicMode, FindLineMode, FindFirstMode, FindSecondMode, FindColorMode, SportModeOn
    move.setup()
    findline.setup()
    ...
    findfirst_threading=threading.Thread(target=findfirst_thread)
    findfirst_threading.setDaemon(True)
    findfirst_threading.start()

    findsecond_threading=threading.Thread(target=findsecond_thread)
    findsecond_threading.setDaemon(True)
    findsecond_threading.start()
    ...
    while True: 
        ...
        elif 'FindFirst' in data:
            FindFirstMode = 1
            FindLineMode = 0
            FindSecondMode = 0
            tcpCliSock.send(('FindFirst').encode())

        elif 'FindSecond' in data:
            FindSecondMode = 1
            FindLineMode = 0
            FindFirstMode = 0
            tcpCliSock.send(('FindSecond').encode())
